...
export function SplitPdf() {
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/pdf/split', getFetchOptions({
        method: 'POST',
        body: formData,
      }));
  ...
  const handleDownloadWithWatermark = (url: string, index: number) => {
    const a = document.createElement('a');
    a.href = url;
    a.download = `split-page-${index + 1}-watermarked.pdf`;
  ...
  const handleDownloadWithoutWatermark = async (url: string, index: number) => {
    if (isFree) {
      try {
        const formData = new FormData();
        formData.append('file', file!);
        ...
        handleDownloadWithWatermark(url, index);
      }
    } else {
      const a = document.createElement('a');
      a.href = url;
    ...
    <ToolLayout toolId="split">
      <PostToolAdGate
        ...
        onDownloadWithWatermark={() => {
          if (results.length > 0) handleDownloadWithWatermark(results[0], 0);
        }}
        onDownloadWithoutWatermark={() => {
          if (results.length > 0) handleDownloadWithoutWatermark(results[0], 0);
        }}
              ...
              <Card className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => { clearUploadedFiles(); setResults([]); setHasOutput(false); }}
                  >
                    Change
                  </Button>
                </div>
                    ...
                    {results.map((url, i) => (
                      <Card key={i} className="flex items-center justify-between p-3">
                        ...
                        <Button
                          ...
                          onClick={() => isFree ? handleDownloadWithWatermark(url, i) : handleDownloadWithoutWatermark(url, i)}
                        >
